...
| `libgcc_s.so.1` | needs Alpine's `libgcc` package | `gcc_s.` is not in the reserved list |
...
1. No `DT_NEEDED` outside the musl-reserved set, except `libgcc_s.so.1`.
...
```sh
# 1. DT_NEEDED — anything not in the reserved set (plus libgcc_s.so.1) is a problem
...
|---|---|
| `-static-libgcc` to drop `libgcc_s.so.1` | **no-op here.** The library links with the C driver, so the `_Unwind_*` symbols pulled in by `libstdc++.a` stay `GLOBAL UND` and `libgcc_s.so.1` is genuinely required — removing that entry breaks the library. It works in isolation (`g++ -shared -static-libstdc++ -l:libstdc++.a -static-libgcc`), which makes it look like it should work here. Alpine satisfies `libgcc_s.so.1` via the `libgcc` package, which the JDK images already install. |
